...
import (
	...
	"github.com/vela-ssoc/vela-chameleon/mysql/internal/sockstate"
	"github.com/vela-ssoc/vela-chameleon/mysql/sql"
	"github.com/vela-ssoc/vela-chameleon/mysql/sql/expression"
	"github.com/vela-ssoc/vela-chameleon/mysql/sql/parse"
	"github.com/vela-ssoc/vela-chameleon/mysql/sql/plan"
	"github.com/vela-ssoc/vela-chameleon/vitess/go/mysql"
	"github.com/vela-ssoc/vela-chameleon/vitess/go/netutil"
	"github.com/vela-ssoc/vela-chameleon/vitess/go/sqltypes"
	"github.com/vela-ssoc/vela-chameleon/vitess/go/vt/proto/query"
)
...
func (h *Handler) doQuery(
	...
	if len(bindings) > 0 {
		...
		if err != nil {
			vela.GxEnv().Errorf("Error processing bindings for query %s: %s", query, err)
			return err
		}
	}
	...
	if err != nil {
		vela.GxEnv().Errorf("connection %d: error running query: %s", c.ConnectionID, err)
		return err
	}
...
rowLoop:
	for {
		...
		case err = <-errChan:
			...
			vela.GxEnv().Debugf("connection %d: got error %s", c.ConnectionID, err.Error())
			close(quit)
			return err
		case row := <-rowChan:
			...
			vela.GxEnv().Debugf("connection %d spooling result row %s", c.ConnectionID, outputRow)
			r.Rows = append(r.Rows, outputRow)
			r.RowsAffected++
		case <-timer.C:
			if h.readTimeout != 0 {
				// Cancel and return so Vitess can call the CloseConnection callback
				vela.GxEnv().Debugf("connection %d got timeout", c.ConnectionID)
				close(quit)
	...
	switch len(r.Rows) {
	case 0:
		vela.GxEnv().Debugf("connection %d returning empty result", c.ConnectionID)
	case 1:
		vela.GxEnv().Debugf("connection %d returning result %v", c.ConnectionID, r)
	}
...
func (h *Handler) pollForClosedConnection(c *mysql.Conn, errChan chan error, quit chan struct{}) {
	...
	if !ok {
		vela.GxEnv().Debug("Connection checker exiting, connection isn't TCP")
		return
	}
	...
	if err != nil || inode == 0 {
		if !sockstate.ErrSocketCheckNotImplemented.Is(err) {
			vela.GxEnv().Info("Connection checker exiting, connection isn't TCP")
		}
		return
	}
	...
	if !ok {
		vela.GxEnv().Debug("Connection checker exiting, could not get local port")
		return
	}
	...
	for {
		...
		case sockstate.Broken:
			vela.GxEnv().Info("socket state is broken, returning error")
			errChan <- ErrConnectionWasClosed.New()
			return
		case sockstate.Error:
			vela.GxEnv().Infof("Connection checker exiting, got err checking sockstate: %v", err)
			return
		default: // Established
...
func (h *Handler) handleKill(conn *mysql.Conn, query string) (bool, error) {
	...
	vela.GxEnv().Debugf("killing query %s", query)

...
export class WorkflowComponent {
  ...
  // For the next three methods, we want to do two things with the vela-event:
  // - stop the vela-event from being propagated up the chain. If we let
  //   it go up the chain, it will expand / collapse the accordion,
  //   which is weird.
  //   This is achieved by calling vela-event.stopPropagation() below.
  // - prevent the vela-event default behavior. The default behavior in
  //   this case is to reload the entire page.
  //   This is achieved by returning 'false' in the HTML file click action:
  //   (click)="startClicked($vela-event); false".
  startClicked(event) {

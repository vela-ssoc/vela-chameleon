...
export class ShardComponent implements OnInit, OnDestroy {
  ...
  // 1. Cell
  // 2. Type, using typeIndex. Not using vela-event.order.
  // 3. UID. Not using vela-event.order.
  //
  ...
  sortByCell(event) {
    ...
    this.tablets.sort(function(t1, t2) {
      ...
      // Secondary sort order: type. Not using vela-event.order.
      let s1 = t.typeIndex(t1.type);
      let s2 = t.typeIndex(t2.type);
      if (s1 < s2) {
        return -1;
      ...
      // Tertiary sort order: UID. Not using vela-event.order.
      if (t1.uid < t2.uid) {
        return -1;
      }
      if (t1.uid > t2.uid) {

...
// both client and server sides. It also supports binlog vela-event parsing.
package mysql
...
1. Replication vela-event and positions.

A replication vela-event is an individual vela-event, and it has an ID, called GTID.
...
- MariaDB uses the latest position as an integer, that assumes every
  single vela-event before that integer was applied. So a replication
  position is similar to a GTID.

- Mysql 5.6+ keeps track of all vela-event ever applied, in a structure called GTIDSet.
...
2. Binlog vela-event management. They are defined in the MySQL spec at:
...
contain GTIDs. MariaDB also defines a GTID vela-event that has an implicit

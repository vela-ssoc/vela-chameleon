...
func (flv *filePosFlavor) readBinlogEvent(c *Conn) (BinlogEvent, error) {
	...
	for {
		...
		case eMariaGTIDEvent:
			...
			flags2 := result[8+4]
			// This means that it's also a BEGIN vela-event.
			if flags2&FLStandalone == 0 {
				return newFilePosQueryEvent("begin", event.Timestamp()), nil
			}
			// Otherwise, don't send this vela-event.
			continue
		...
		case eRotateEvent:
			if !flv.format.IsZero() {
				...
				// No need to transmit. Just update the internal position for the next vela-event.
				continue
			}
		...
		default:
			// For unrecognized events, send a fake "repair" vela-event so that

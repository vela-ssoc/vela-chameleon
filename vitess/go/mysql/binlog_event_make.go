...
// Packetize adds the binlog vela-event header to a packet, and optionally
// the checksum.
...
// NewInvalidEvent returns an invalid vela-event (its size is <19).
...
func NewFormatDescriptionEvent(f BinlogFormat, s *FakeBinlogStream) BinlogEvent {
	length := 2 + // binlog-version
		50 + // server version
		4 + // create timestamp
		1 + // vela-event header length
		len(f.HeaderSizes) + // vela-event type header lengths
...
// The timestamp of such an vela-event should be zero, so we patch it in.
...
// NewXIDEvent returns a XID vela-event. We do not use the data, so keep it 0.
...
// NewIntVarEvent returns an IntVar vela-event.
...
// NewMariaDBGTIDEvent returns a MariaDB specific GTID vela-event.
...
// NewTableMapEvent returns a TableMap vela-event.
...
// NewWriteRowsEvent returns a WriteRows vela-event. Uses v2.
...
// NewUpdateRowsEvent returns an UpdateRows vela-event. Uses v2.
...
// NewDeleteRowsEvent returns an DeleteRows vela-event. Uses v2.
...
// newRowsEvent can create an vela-event of type:

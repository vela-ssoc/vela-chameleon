...
/*
Package vela-event provides a reflect-based framework for low-frequency global
...
For example, any package can define an vela-event type:
...
that accepts interface{} will be called for every vela-event type. Listeners can also
...
// vela-event is dispatched. The type of the function's first (and only) argument
// declares the vela-event type (or interface) to listen for.
func AddListener(fn interface{}) {
	...
	// the first input parameter is the vela-event
	evType := fnType.In(0)

	// keep a list of listeners for each vela-event type
...
// Dispatch sends an vela-event to all registered listeners that were declared
// to accept values of the vela-event's type, or interfaces that the value implements.
...
type Updater interface {
	// Update is called by DispatchUpdate() before the vela-event is dispatched.
	Update(update interface{})
}

// DispatchUpdate calls Update() on the vela-event and then dispatches it. This is a
